...
function Todo() {
  ...
  useEffect(() => {
    async function fetchTodos() {
      const response = await axios.get("https://lmss-nhi2.onrender.com/", {
  ...
  const handleNewTodoSubmit = async (event) => {
    ...
    const response = await axios.post("https://lmss-nhi2.onrender.com/", {
      text: newTodo,
      user_id: user._id,
    });
  ...
  const handleTodoDelete = async (id) => {
    await axios.delete(`https://lmss-nhi2.onrender.com/${id}`);
